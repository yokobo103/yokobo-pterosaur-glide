...
const r = await p.evaluate(() => {
  ...
  const run = (dir) => {
    ...
    const hL = v.world(mark[0]-4000, 120, mark[2]), hR = v.world(mark[0]+4000, 120, mark[2]);
    ...
    return { markMove: after - before, rightWingLower: right[1] - left[1],
             horizonRightUp: hL[1] - hR[1], noseVsVel: Math.acos(Math.max(-1,Math.min(1,dot)))*57.3,
...
console.log(`  地平線は右側が ${r.right.horizonRightUp > 0 ? '上がった' : '下がった'} ${ok(r.right.horizonRightUp > 0)}`);
console.log('■ 左を押したとき');
